...
func (this *Reader) Init(runner core.IRunner, reader core.IReader, meta core.IMetaerData, options core.IReaderOptions) (err error) {
	...
	if this.schemas, err = SchemaCheck(this.options.GetMysql_schema()); err != nil {
		return
	}
...
func (this *Reader) Start() (err error) {
	...
	if !this.options.GetAutoClose() {
		if this.runId, err = lgcron.AddFunc(this.options.GetMysql_batch_intervel(), this.scanSql); err != nil {
			err = fmt.Errorf("lgcron.AddFunc corn:%s err:%v", this.options.GetMysql_batch_intervel(), err)
			return
		}
		if this.options.GetMysql_exec_onstart() {
			go this.scanSql()
		}
	} else {
		go this.scanSql()
...
func (this *Reader) scanSql() {
	...
	if tables, err = this.scanddatabase(); err == nil && len(tables) > 0 {
		...
		if this.options.GetMysql_collection_type() == StaticTableCollection { //静态表采集
			for _, k := range this.options.GetMysql_tables() {
				if v, ok := tables[k]; ok {
					...
					this.Runner.Errorf("Reader Mysql not found table：%s", k)
				}
			}
		} else if this.options.GetMysql_collection_type() == DynamicTableCollection { //动态表
			var (
				nextDate    string
				canIncrease bool
				dynamictame string
			...
			locp:
				for {
					nextDate, canIncrease = GetNextDate(this.options.GetMysql_datetimeFormat(), this.options.GetMysql_startDate(), int(dynamictable.TableAlreadyReadOffset), this.options.GetMysql_dateUnit())
					if canIncrease {
						dynamictable.TableAlreadyReadOffset += uint64(this.options.GetMysql_dateGap())
						...
						if v, ok := tables[dynamictame]; ok {
							...
							this.Runner.Errorf("Reader Mysql not found table：%s", dynamictame)
						}
					} else {
						this.SyncMeta()
						break locp
		...
		if len(collectiontables) > 0 {
			...
			this.Runner.Debugf("Reader Mysql start new collection:%d", atomic.AddInt32(&this.rungoroutine, int32(procs)))
			this.wg.Add(procs)
			for i := 0; i < procs; i++ {
		...
		if this.options.GetAutoClose() { //自动关闭
			...
			if strings.Contains(err.Error(), "database is closed") && this.Runner.GetRunnerState() == core.Runner_Runing {
				this.sql.Close()
				if this.sql, err = lgsql.NewSys(lgsql.SetSqlType(lgsql.MySql), lgsql.SetSqlUrl(fmt.Sprintf("%s/%s", this.options.GetMysql_datasource(), this.options.GetMysql_database()))); err != nil {
...
func (this *Reader) scanddatabase() (tables map[string]uint64, err error) {
	...
	sql = fmt.Sprintf("select table_name from information_schema.tables where table_schema = '%s' order by table_rows asc", this.options.GetMysql_database())
...
func (this *Reader) getsqlstr(table *msql.TableMeta) (sqlstr string) {
	sqlstr = strings.Replace(this.options.GetMysql_sql(), "$TABLE$", table.TableName, -1)
	sqlstr = fmt.Sprintf("%s limit %d,%d;", sqlstr, table.TableAlreadyReadOffset, table.TableAlreadyReadOffset+uint64(this.options.GetMysql_limit_batch()))
	// if this.options.GetMysql_offset_key() == "INCREMENTAL" {
	// 	sqlstr = fmt.Sprintf("SELECT * FROM (%s,(select @rowno:=0) t)t WHERE %s >= %d order by %s limit %d;", sqlstr, this.options.GetMysql_offset_key(), table.TableAlreadyReadOffset, this.options.GetMysql_offset_key(), this.options.GetMysql_limit_batch())
	// } else { //有自己饿组件
	// 	sqlstr = fmt.Sprintf("%s WHERE %s >= %d order by %s limit %d;", sqlstr, this.options.GetMysql_offset_key(), table.TableAlreadyReadOffset, this.options.GetMysql_offset_key(), this.options.GetMysql_limit_batch())
	// }
	return
}
...
func (this *Reader) writeDataChan(table *msql.TableMeta, data map[string]interface{}) (isend bool) {
	...
	this.Input() <- core.NewCollData(this.options.GetMysql_ip(), data)
