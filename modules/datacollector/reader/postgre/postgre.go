...
func (this *Reader) Init(runner core.IRunner, reader core.IReader, meta core.IMetaerData, options core.IReaderOptions) (err error) {
	...
	if this.schemas, err = SchemaCheck(this.options.GetPostgre_schema()); err != nil {
		return
	}
	if this.sql, err = this.connection(); err != nil {
		return
	}
	return
}
func (this *Reader) connection() (db *sql.DB, err error) {
	psqlInfo := fmt.Sprintf("host=%s port=%d user=%s "+
		"password=%s dbname=%s sslmode=disable",
		this.options.GetPostgre_ip(), this.options.GetPostgre_port(), this.options.GetPostgre_user(), this.options.GetPostgre_password(), this.options.GetPostgre_database())
	db, err = sql.Open("postgres", psqlInfo)
	if err != nil {
		return nil, fmt.Errorf("Reader postgres connection sql.Open err:%v", err)
	}
	err = db.Ping()
	if err != nil {
		return nil, fmt.Errorf("Reader postgres connection Ping err:%v", err)
	}
...
func (this *Reader) Start() (err error) {
	...
	if !this.options.GetAutoClose() {
		if this.runId, err = lgcron.AddFunc(this.options.GetPostgre_batch_intervel(), this.scanSql); err != nil {
			err = fmt.Errorf("lgcron.AddFunc corn:%s err:%v", this.options.GetPostgre_batch_intervel(), err)
			return
		}
		if this.options.GetPostgre_exec_onstart() {
			go this.scanSql()
		}
	} else {
		go this.scanSql()
...
func (this *Reader) scanSql() {
	...
	if tables, err = this.scanddatabase(); err == nil && len(tables) > 0 {
		...
		if this.options.GetPostgre_collection_type() == StaticTableCollection { //静态表采集
			for _, k := range this.options.GetPostgre_tables() {
				if v, ok := tables[k]; ok {
					...
					this.Runner.Errorf("Reader Postgre not found table：%s", k)
				}
			}
		} else if this.options.GetPostgre_collection_type() == DynamicTableCollection { //动态表
			var (
				nextDate    string
				canIncrease bool
				dynamictame string
			...
			locp:
				for {
					nextDate, canIncrease = GetNextDate(this.options.GetPostgre_datetimeFormat(), this.options.GetPostgre_startDate(), int(dynamictable.TableAlreadyReadOffset), this.options.GetPostgre_dateUnit())
					if canIncrease {
						dynamictable.TableAlreadyReadOffset += uint64(this.options.GetPostgre_dateGap())
						...
						if v, ok := tables[dynamictame]; ok {
							...
							this.Runner.Errorf("Reader Postgre not found table：%s", dynamictame)
						}
					} else {
						this.SyncMeta()
						break locp
		...
		if len(collectiontables) > 0 {
			...
			this.Runner.Debugf("Reader Postgre start new collection:%d", atomic.AddInt32(&this.rungoroutine, int32(procs)))
			this.wg.Add(procs)
			for i := 0; i < procs; i++ {
...
func (this *Reader) asyncollection(db *sql.DB, fmeta <-chan *msql.TableMeta) {
	...
	if data, err = this.sql.Query(sql); err == nil {
		...
		for data.Next() {
			if e := data.Scan(&tablename); e == nil {
				...
				this.Runner.Errorf("Reader scanddatabase %s err:%v", sql, err)
			}
		}
	} else {
		this.Runner.Errorf("scanddatabase:%v", err)
	}
...
//采集数据表
func (this *Reader) collection_table(db *sql.DB, table *msql.TableMeta) (isend bool, err error) {
	var (
		sqlstr   string
		data     *sql.Rows
		columns  []string
...
func (this *Reader) gettablecount(tablename string) (count uint64) {
	...
	fmt.Println("gettablecount:%v", count)
	return
}
...
func (this *Reader) getsqlstr(table *msql.TableMeta) (sqlstr string) {
	sqlstr = strings.Replace(this.options.GetPostgre_sql(), "$TABLE$", table.TableName, -1)
	//sqlstr = fmt.Sprintf("%s limit %d,%d;", sqlstr, table.TableAlreadyReadOffset, table.TableAlreadyReadOffset+uint64(this.options.GetPostgre_limit_batch()))
	endnum := table.TableAlreadyReadOffset + uint64(this.options.GetPostgre_limit_batch())
	if this.options.GetPostgre_offset_key() == "INCREMENTAL" {
		sqlstr = fmt.Sprintf("select * from (%s) as t where INCREMENTAL between %d and %d", sqlstr, table.TableAlreadyReadOffset+1, endnum)
	} else { //有自己主键id
		sqlstr = strings.Replace(sqlstr, "select 0", this.options.GetPostgre_offset_key(), -1)
		sqlstr = fmt.Sprintf("select * from (%s) as t where %s between %d and %d", sqlstr, this.options.GetPostgre_offset_key(), table.TableAlreadyReadOffset, endnum)
	}
	return
}
...
func (this *Reader) writeDataChan(table *msql.TableMeta, data map[string]interface{}) (isend bool) {
	// this.Runner.Debugf("reder mssql table:%s writeDataChan start", table.TableName)
	// defer this.Runner.Debugf("reder mssql table:%s writeDataChan end", table.TableName)
	// conf := this.options.GetPostgre_datasource()
	// addr := conf[strings.Index(conf, "=")+1 : strings.Index(conf, ";")]
	this.Input() <- core.NewCollData(this.options.GetPostgre_ip(), data)
