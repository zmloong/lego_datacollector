...
func (this *Reader) Init(runner core.IRunner, reader core.IReader, meta core.IMetaerData, options core.IReaderOptions) (err error) {
	...
	if this.schemas, err = SchemaCheck(this.options.GetMssql_schema()); err != nil {
		return
	}
	this.sql, err = lgsql.NewSys(lgsql.SetSqlType(lgsql.SqlServer), lgsql.SetSqlUrl(fmt.Sprintf("%s", this.options.GetMssql_datasource())))
	return
}
...
func (this *Reader) Start() (err error) {
	...
	if !this.options.GetAutoClose() {
		if this.runId, err = lgcron.AddFunc(this.options.GetMssql_batch_intervel(), this.scanSql); err != nil {
			err = fmt.Errorf("lgcron.AddFunc corn:%s err:%v", this.options.GetMssql_batch_intervel(), err)
			return
		}
		if this.options.GetMssql_exec_onstart() {
			go this.scanSql()
		}
	} else {
		go this.scanSql()
...
func (this *Reader) scanSql() {
	...
	if tables, err = this.scanddatabase(); err == nil && len(tables) > 0 {
		...
		if this.options.GetMssql_collection_type() == StaticTableCollection { //静态表采集
			for _, k := range this.options.GetMssql_tables() {
				if v, ok := tables[k]; ok {
					...
					this.Runner.Errorf("Reader Mssql not found table：%s", k)
				}
			}
		} else if this.options.GetMssql_collection_type() == DynamicTableCollection { //动态表
			var (
				nextDate    string
				canIncrease bool
				dynamictame string
			...
			locp:
				for {
					nextDate, canIncrease = GetNextDate(this.options.GetMssql_datetimeFormat(), this.options.GetMssql_startDate(), int(dynamictable.TableAlreadyReadOffset), this.options.GetMssql_dateUnit())
					if canIncrease {
						dynamictable.TableAlreadyReadOffset += uint64(this.options.GetMssql_dateGap())
						...
						if v, ok := tables[dynamictame]; ok {
							...
							this.Runner.Errorf("Reader Mssql not found table：%s", dynamictame)
						}
					} else {
						this.SyncMeta()
						break locp
		...
		if len(collectiontables) > 0 {
			...
			this.Runner.Debugf("Reader Mssql start new collection:%d", atomic.AddInt32(&this.rungoroutine, int32(procs)))
			this.wg.Add(procs)
			for i := 0; i < procs; i++ {
		...
		if this.options.GetAutoClose() { //自动关闭
			...
			if strings.Contains(err.Error(), "database is closed") && this.Runner.GetRunnerState() == core.Runner_Runing {
				this.sql.Close()
				if this.sql, err = lgsql.NewSys(lgsql.SetSqlType(lgsql.SqlServer), lgsql.SetSqlUrl(fmt.Sprintf("%s", this.options.GetMssql_datasource()))); err != nil {
...
func (this *Reader) scanddatabase() (tables map[string]uint64, err error) {
	...
	sql = fmt.Sprintf("select name from sysobjects where xtype='u'")
...
func (this *Reader) getsqlstr(table *msql.TableMeta) (sqlstr string) {
	sqlstr = strings.Replace(this.options.GetMssql_sql(), "$TABLE$", table.TableName, -1)
	//sqlstr = fmt.Sprintf("%s limit %d,%d;", sqlstr, table.TableAlreadyReadOffset, table.TableAlreadyReadOffset+uint64(this.options.GetMssql_limit_batch()))
	endnum := table.TableAlreadyReadOffset + uint64(this.options.GetMssql_limit_batch())
	if this.options.GetMssql_offset_key() == "INCREMENTAL" {
		sqlstr = fmt.Sprintf("select * from (%s) as t where INCREMENTAL between %d and %d", sqlstr, table.TableAlreadyReadOffset+1, endnum)
	} else { //有自己主键id
		sqlstr = strings.Replace(sqlstr, "select 0", this.options.GetMssql_offset_key(), -1)
		sqlstr = fmt.Sprintf("select * from (%s) as t where %s between %d and %d", sqlstr, this.options.GetMssql_offset_key(), table.TableAlreadyReadOffset, endnum)
	}
	return
}
...
func (this *Reader) writeDataChan(table *msql.TableMeta, data map[string]interface{}) (isend bool) {
	...
	conf := this.options.GetMssql_datasource()
	addr := conf[strings.Index(conf, "=")+1 : strings.Index(conf, ";")]
	this.Input() <- core.NewCollData(addr, data)
