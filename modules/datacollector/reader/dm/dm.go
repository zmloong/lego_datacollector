...
func (this *Reader) Init(runner core.IRunner, reader core.IReader, meta core.IMetaerData, options core.IReaderOptions) (err error) {
	...
	this.ipaddr = this.options.GetDM_datasource()[strings.Index(this.options.GetDM_datasource(), "@")+1 : strings.LastIndex(this.options.GetDM_datasource(), ":")]
	this.colltask = make(chan *msql.TableMeta)
	if this.schemas, err = SchemaCheck(this.options.GetDM_schema()); err != nil {
		return
	}
	this.sql, err = lgsql.NewSys(lgsql.SetSqlType(lgsql.DM), lgsql.SetSqlUrl(this.options.GetDM_datasource()))
	return
}
...
func (this *Reader) Start() (err error) {
	...
	if !this.options.GetAutoClose() {
		if this.runId, err = lgcron.AddFunc(this.options.GetDM_batch_intervel(), this.scanSql); err != nil {
			err = fmt.Errorf("lgcron.AddFunc corn:%s err:%v", this.options.GetDM_batch_intervel(), err)
			return
		}
		if this.options.GetDM_exec_onstart() {
			go this.scanSql()
		}
	} else {
		go this.scanSql()
...
func (this *Reader) scanSql() {
	...
	if tables, err = this.scanddatabase(); err == nil && len(tables) > 0 {
		...
		if this.options.GetDM_collection_type() == StaticTableCollection { //静态表采集
			for _, k := range this.options.GetDM_tables() {
				if _, ok := tables[k]; ok {
					...
					this.Runner.Errorf("Reader DM not found table：%s", k)
				}
			}
		} else if this.options.GetDM_collection_type() == DynamicTableCollection { //动态表
			var (
				nextDate    string
				canIncrease bool
				dynamictame string
			...
			locp:
				for {
					nextDate, canIncrease = GetNextDate(this.options.GetDM_datetimeFormat(), this.options.GetDM_startDate(), int(dynamictable.TableAlreadyReadOffset), this.options.GetDM_dateUnit())
					if canIncrease {
						dynamictable.TableAlreadyReadOffset += uint64(this.options.GetDM_dateGap())
						...
						if _, ok := tables[dynamictame]; ok {
							...
							this.Runner.Errorf("Reader DM not found table：%s", dynamictame)
						}
					} else {
						this.SyncMeta()
						break locp
		...
		if this.options.GetAutoClose() { //自动关闭
			...
			atomic.StoreInt32(&this.isend, 0)
			if strings.Contains(err.Error(), "网络通信异常") && this.Runner.GetRunnerState() == core.Runner_Runing {
				this.sql.Close()
				if this.sql, err = lgsql.NewSys(lgsql.SetSqlType(lgsql.DM), lgsql.SetSqlUrl(this.options.GetDM_datasource())); err != nil {
	...
	if data, err = this.sql.Query(sql); err == nil {
		...
		for data.Next() {
			if e := data.Scan(&tablename); e == nil {
				...
				// this.Runner.Warnf("Reader scanddatabase %s err:%v", sql, err)
			}
		}
	}
	return
...
func (this *Reader) gettablecount(tablename string) (count uint64) {
	...
	count = 0
	sqlstr = fmt.Sprintf(`select count(*) from %s.%s`, this.options.GetDM_database(), tablename)
	if data, err = this.sql.Query(sqlstr); err != nil {
...
func (this *Reader) getsqlstr(table *msql.TableMeta) (sqlstr string) {
	sqlstr = strings.Replace(this.options.GetDM_sql(), "$TABLE$", fmt.Sprintf("%s.%s", this.options.GetDM_database(), table.TableName), -1)
	sqlstr = fmt.Sprintf("%s limit %d,%d;", sqlstr, table.TableAlreadyReadOffset, table.TableAlreadyReadOffset+uint64(this.options.GetDM_limit_batch()))
	return
}
...
func (this *Reader) writeDataChan(table *msql.TableMeta, data map[string]interface{}) (isend bool) {
	this.Input() <- core.NewCollData(this.ipaddr, data)
