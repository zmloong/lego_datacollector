...
func (this *Reader) sftp_collection() {
	...
	if conn, err := this.sftp_connectSftp(); err != nil {
		...
		this.sftp_scandirectory(conn, this.options.GetBinary_directory(), fmetas, cfiles)
	}
	if len(cfiles) > 0 {
...
func (this *Reader) sftp_connectSftp() (conn *sftp.Client, err error) {
	...
	clientConfig := &ssh.ClientConfig{
		User:            this.options.GetBinary_server_user(),
		Auth:            []ssh.AuthMethod{ssh.Password(this.options.GetBinary_server_password())},
		Timeout:         10 * time.Second,
		HostKeyCallback: ssh.InsecureIgnoreHostKey(),
	}
	if sshClient, err = ssh.Dial("tcp", fmt.Sprintf("%s:%d", this.options.GetBinary_server_addr(), this.options.GetBinary_server_port()), clientConfig); err != nil {
		return
	}
...
func (this *Reader) sftp_collection_file(client *sftp.Client, f *file.FileMeta) (isend bool, err error) {
	...
	if _file, err = client.Open(f.FileName); err == nil {
		defer _file.Close()
		if n, err = _file.ReadAt(buff, int64(f.FileAlreadyReadOffset)+int64(len(f.FileCacheData))); err != nil && err != io.EOF {
			return
		}
	}
	if n > 0 {
