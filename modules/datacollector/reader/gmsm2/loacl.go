...
func (this *Reader) local_collection() {
	...
	if err := this.local_scanDirectory(this.options.GetGmsm2_directory(), files); err == nil {
		for k, v := range files {
	...
	if fs, err = ioutil.ReadDir(_path); err == nil {
		for _, v := range fs {
			...
			if v.IsDir() { //是目录
				...
				if len(files) >= this.options.GetGmsm2_max_collection_num() {
					this.Runner.Debugf("Reader scanDirectory up top")
					return
				} else if v.Size() > this.options.GetGmsm2_max_collection_size() {
					this.Runner.Debugf("Reader file:%s size up top", v.Name())
				} else {
					if matched, err = regexp.MatchString(this.options.GetGmsm2_regularrules(), v.Name()); err == nil && matched {
						files[fp] = v
					} else {
						this.Runner.Debugf("Reader regular:%s file:%s matched:%v err:%v", this.options.GetGmsm2_regularrules(), v.Name(), matched, err)
					}
				}
			}
