...
func (this *Reader) ftp_collection() {
	...
	if conn, err := this.ftp_connectFtp(); err != nil {
		...
		this.ftp_scandirectory(conn, this.options.GetGmsm2_directory(), fmetas, cfiles)
		conn.Quit()
	}
...
func (this *Reader) ftp_connectFtp() (conn *ftp.ServerConn, err error) {
	if conn, err = ftp.Dial(fmt.Sprintf("%s:%d", this.options.GetGmsm2_server_addr(), this.options.GetGmsm2_server_port()), ftp.DialWithTimeout(5*time.Second)); err == nil {
		err = conn.Login(this.options.GetGmsm2_server_user(), this.options.GetGmsm2_server_password())
	}
	return
}
			...
			if f, ok := fmetas[filepath]; !ok || f.FileLastCollectionTime < entrie.Time.Unix() { //文件有修改
				...
				if entrie.Type == ftp.EntryTypeFolder {
					...
					if len(cfile) >= this.options.GetGmsm2_max_collection_num() {
						...
						return
					} else if int64(entrie.Size) > this.options.GetGmsm2_max_collection_size() {
